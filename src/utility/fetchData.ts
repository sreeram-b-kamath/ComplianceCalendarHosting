...
export const useFetchData = async (): Promise<FilingData[]> => {
    try {
        ...
        let response;
        if (RoleName === "Admin") {
            response = await axios.get<FilingData[]>(`https://localhost:7013/Filings/GetAdminFilings/${EmployeeId}`);
        } else {
            response = await axios.get<FilingData[]>(`https://localhost:7013/Filings/GetUserFilings/${EmployeeId}`);
        }
